'use strict';
 ...
 exports.dialogflowFirebaseFulfillment = functions.https.onRequest((request, response) => {
   ...
   function makeAppointment (agent) {
    ...
    // );
     ...
     //additional
       const userAskedDate=agent.parameters.date.split('T')[0];
      return listEvents(userAskedDate,agent).then(function(value) {
        console.log("hey");
        console.log(" then vaalue is",value);
       agent.add(`Ok, let me see if we can fit you in is fine!.`);
     }).catch(() => {
         console.log(" then vaalue is in catch");
 ...
 function listEvents(userAskedDate,agent) {
   agent.add(`hey im 1st`);
   return new Promise((resolve, reject) => {
        agent.add(`hey im 2st`);
